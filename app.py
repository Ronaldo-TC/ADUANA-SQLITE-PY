...
def reportes():
    ...
    reporte = [dict(row) for row in reporte]
...
def exportar_reportes():
    ...
    # Determina el tipo de reporte y construye la consulta
    if tipo_reporte == "operaciones":
        query = "SELECT id_operacion, descripcion, estado, fecha FROM operaciones WHERE id_usuario = ?"
        params.append(session["user_id"])
    elif tipo_reporte == "cotizaciones":
    ...
    # Agrega filtros por fecha si están presentes
    if fecha_inicio:
        query += " AND fecha >= ?"
        params.append(fecha_inicio)
    if fecha_fin:
    ...
    if formato == "csv":
        output = BytesIO()
        pd.DataFrame(datos).to_csv(output, index=False)
        output.seek(0)
        return Response(output, mimetype="text/csv",
                        headers={"Content-Disposition": f"attachment;filename={tipo_reporte}.csv"})
    ...
    if formato == "xlsx":
        ...
        output.seek(0)
        return Response(output, mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        headers={"Content-Disposition": f"attachment;filename={tipo_reporte}.xlsx"})

    # Exportar a PDF
    if formato == "pdf":
        output = BytesIO()
        pdf = SimpleDocTemplate(output, pagesize=landscape(letter))
        pdf_data = [[key for key in datos[0].keys()]]  # Encabezados
        for row in datos:
            pdf_data.append([str(value) for value in row.values()])
        ...
        output.seek(0)
        return Response(output, mimetype="application/pdf",
                        headers={"Content-Disposition": f"attachment;filename={tipo_reporte}.pdf"})
...
def configuracion():
    ...
    conn = sqlite3.connect("plataforma.db")
    conn.row_factory = sqlite3.Row  # Convierte las filas en objetos tipo diccionario
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM usuarios WHERE id_usuario = ?",
                   (session["user_id"],))
    usuario = cursor.fetchone()  # Devuelve una fila como objeto tipo diccionario
    conn.close()

    return render_template("configuracion/index.html", usuario=usuario)


# @app.route("/configuracion/perfil", methods=["POST"])
...
#         WHERE id_usuario = ?
#     """, (nombre_completo, correo, session["user_id"]))
#     conn.commit()
#     conn.close()
...
def actualizar_perfil():
    ...
    return redirect("/configuracion?success=Perfil actualizado correctamente")


# @app.route("/configuracion/seguridad", methods=["POST"])
...
#         return redirect("/configuracion?error=ContrasenasNoCoinciden")

#     conn = sqlite3.connect("plataforma.db")
#     conn.row_factory = sqlite3.Row
#     cursor = conn.cursor()
#     cursor.execute(
#         "SELECT contrasena FROM usuarios WHERE id_usuario = ?", (session["user_id"],))
#     usuario = cursor.fetchone()

#     if not usuario:
#         return redirect("/configuracion?error=UsuarioNoEncontrado")

#     if not check_password_hash(usuario["contrasena"], contrasena_actual):
#         return redirect("/configuracion?error=ContrasenaIncorrecta")

#     nueva_contrasena_hash = generate_password_hash(nueva_contrasena)
#     cursor.execute("""
#         UPDATE usuarios SET contrasena = ?
#         WHERE id_usuario = ?
#     """, (nueva_contrasena_hash, session["user_id"]))
#     conn.commit()
#     conn.close()

#     return redirect("/configuracion?success=ContrasenaActualizada")
...
def cambiar_contrasena():
    ...
    if nueva_contrasena != confirmar_contrasena:
        return redirect("/configuracion?error=Las contraseñas no coinciden")
    ...
    if not usuario:
        return redirect("/configuracion?error=Usuario no encontrado")

    if not check_password_hash(usuario["contrasena"], contrasena_actual):
        return redirect("/configuracion?error=Contraseña actual incorrecta")
    ...
    return redirect("/configuracion?success=Contraseña actualizada correctamente")
